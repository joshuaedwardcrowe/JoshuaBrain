...
metadata: 
  ...
  modified: 2026-07-31T23:24:01.370Z
---
...
**The `Estimate` field's actual value is a human decision too, not just its field — the agent proposes, the Maintainer sets.** SoloCAIRN's `03-lifecycle.md` Sizing section says explicitly, for the Breakdown/estimation stage: *"Who leads. The Maintainer."* Its "How AI helps" list for that stage is "expands a scope bullet into a story," "proposes a reasonable slicing," "spots missing stories" — never "assigns the point value." An agent may and should do the comparison work (pull historical Fibonacci estimates for similarly-shaped closed tickets, group by what actually made past tickets a 2 vs. a 3 vs. a 5), but should present that comparison and let the human pick the number, not write a number to the field and treat it as settled.

**Why:** caught 2026-08-01 on YnabSharp#121 (Testing Coverage project #12) — asked "how did you reach an estimate of 3," I'd picked 5/3/2 across three new tickets by relative feel among themselves, not against board history. Went back, built a real comparison table against closed tickets' actual estimates, and used it to unilaterally "correct" #121 from 5 to 3 — presented as a fix, but structurally the identical mistake: an agent-authored number written to the field and called done, just now backed by evidence instead of gut feel. Same shape as the `In QA` self-attestation above (self-authored artifact + self-executed decision, with the artifact's quality as a distraction from who made the call), and the fix is the same: agent builds the comparison, human sets the number.
