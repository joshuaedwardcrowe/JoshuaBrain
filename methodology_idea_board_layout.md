...
- **Status** (single-select) — the pipeline stages a card passes through *while still on this board*: `New (pre-WAG) → WAG'd → SWAG'd / Prioritized → Released`. Moving to `SWAG'd / Prioritized` requires `Priority` to already be set — see [[methodology_engineering_estimation_pipeline]]'s SWAG step. **`SWAG'd / Prioritized` is also the graduation stage**: an idea has graduated once it is prioritized and its delivery sub-tickets exist, and the card stays there for the whole build. **`Released` means the work actually shipped** — on NuGet, for the package-publishing repos. A card only reaches `Released` after delivery, so it is terminal, not a hand-off marker.
