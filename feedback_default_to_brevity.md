...
**Laymen's terms outranks matching a sibling's density.** When an existing artifact — a ticket, a doc, a comment thread — is written implementer-dense, resembling it does not excuse jargon; plain language wins and the sibling is the thing off-style. Flagged 2026-08-29 on KitCli #243/#244: ticket bodies drafted to match sibling #190's density ("I cant understand what you just said") when this rule already covered them — tickets are prose like any other. The clash was with `feedback_no_unrequested_deviation`'s "resemble the previous shape", which governs code, not the register of prose.
...
**Recurred a fourth time on 2026-08-25** — a thirty-line CHANGELOG entry where `CONTRIBUTING.md` said "a line", hours after the investigation-0003 flag above. The fix was not more wording: this guidance lives in a file that is only loaded as a one-line summary in `MEMORY.md`, so it is not in context at the moment of writing. It is now also a rule at `~/.claude/rules/writing-markdown.md` with no `paths:` frontmatter, so it loads unconditionally at session start, in any repo — `paths:`-scoped rules trigger on Read only, and in `~/.claude/rules/` not at all (anthropics/claude-code#21858), so scoping this rule to `**/*.md` kept it out of context at exactly the moment it existed for. That rule states the styles themselves rather than pointing at an exemplar repo, and covers every doc kind — README, user guide, concept, investigation, reference entry — not just ADRs. Confirmed 2026-08-25: Strunk, laymen's terms, a fixed section skeleton per `docs/` folder, and under 60 lines per doc whatever the kind. The rule's content is drawn from what five CONTRIBUTING.md files already agree on (KitCli, SpendfulnessCli, YnabSharp, SoloCAIRN, DataTool.Cli) — ADR vs concept vs investigation vs user guide, when to skip an ADR, verify every name and transcript against source, keep docs current in the same PR. Each repo's own CONTRIBUTING stays binding and more specific; the rule exists because it is loaded every session and CONTRIBUTING is not. KitCli's 118-178 line ADRs are off-style, not the benchmark. Keep the two in step; this file stays the full reasoning.
